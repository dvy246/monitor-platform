...
| :--- | :---: | :---: | :--- |
| **Canonical & Domain Alignment** | **35/100** | ❌ FAIL | `robots.txt` points to `monitortesthub.com` while 62+ codebase files hardcode `displaytestonline.com`. |
...
#### Issue P0-1: Canonical Domain & Sitemap URL Mismatch (`displaytestonline.com` vs `monitortesthub.com`)
- **Location**:
...
- **Evidence**:
  ...
  ```
  Consequently, `@astrojs/sitemap` generates sitemaps containing `https://displaytestonline.com/...`. In addition, 62+ occurrences across `src/` hardcode `https://displaytestonline.com`.
- **Why it matters**: Search engine crawlers visiting `monitortesthub.com` are instructed by `robots.txt` to read a sitemap index that lists cross-domain URLs on `displaytestonline.com`. Cross-domain sitemap declarations and canonical URL mismatches cause search engines to reject sitemaps, ignore canonical signals, or drop pages from indexing.
...
- **Evidence**:
  `SchemaGraph.astro` hardcodes `"authorName": "Monitor Test Hub Engineering Team"` pointing to `https://displaytestonline.com/about`. `about.astro` lacks individual author profiles, display engineer bios, or linkable reviewer credentials. Line 19 of `about.astro` also sets `"sameAs": ["https://github.com"]` pointing to generic GitHub instead of a specific repository URL.
...
- **Evidence**:
  Breadcrumb items generated by `SchemaGraph.astro` split `urlObj.pathname` by `/`. For `/es/display-tests/dead-pixel`, it generates intermediate breadcrumb item `https://displaytestonline.com/es` (labeled "Es") and `https://displaytestonline.com/es/display-tests` (labeled "Display Tests").
...
- **Evidence**: `ogImage` defaults to `'https://displaytestonline.com/og-image.png'`.
